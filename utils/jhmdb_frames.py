...
# Assisting function for finding a good/bad tubelet  save
...
class VideoDataset(Dataset):
    ...
    def __init__(self, directory, video_path, transforms, clip_len=8, crop_size=224, resize_size=256,
                 mode='train'):
        self.directory = directory  #anno path
        cache_file = os.path.join(directory, 'UCF101v2-GT_small.pkl')
        assert os.path.isfile(cache_file), "Missing cache file for dataset "

        with open(cache_file, 'rb') as fid:
            dataset = pickle.load(fid, encoding='iso-8859-1')

        self.video_path = video_path    #Frames path
        self._transforms = transforms
        self.dataset = dataset
        self.mode = mode
        self.clip_len = clip_len    #32
        self.crop_size = crop_size  #224
        self.resize_size = resize_size  #256
        self.index_cnt = 0
        ...
        if mode == 'val' or mode == 'test':
            self.dataset_samples = self.dataset['test_videos'][0]   #为什么只取0？
        elif mode == 'train':
    ...
    def __getitem__(self, index):   #index是dataloader所遍历的当前帧
    ...
    def load_annotation(self, sample_id, start, index, p_t):
        ...
        for ilabel, tubes in self.dataset['gttubes'][sample_id].items():
            ...
            for t in tubes:
                box_ = t[(t[:, 0] == start), 0:5]   #看每个t的第一个数(帧id)等不等于start
                key_point = key_pos // 8

                if len(box_) > 0:
                    box = box_[0]
                    p_x1 = int(box[1] / ow * nw)    #根据前面的缩放来进行缩放
                    p_y1 = int(box[2] / oh * nh)
                    p_x2 = int(box[3] / ow * nw)
                    p_y2 = int(box[4] / oh * nh)
                    boxes.append([key_pos, p_x1, p_y1, p_x2, p_y2])
                    classes.append(np.clip(ilabel, 0, 24))  #把ilabel的值限制再0到24
        ...
        if self.mode == 'test' and False:
            ...
            boxes[:, 1::3].clamp_(min=0, max=nw)    #将张量的值限制在范围内
            boxes[:, 2::3].clamp_(min=0, max=nh)

            if boxes.shape[0]:
                raw_boxes = F.pad(boxes, (1, 0, 0, 0), value=self.index_cnt)#扩充一个维度：1，5-> 1,6
            else:
                raw_boxes = boxes
    ...
    def loadvideo(self, mid_point, sample_id, target, p_t):
        ...
        #     print(111)
        start = max(mid_point - p_t, 0) #开始帧 mid point 就是当前帧  todo:如果靠右，左边直接取的全是0
        end = min(mid_point + self.clip_len - p_t, self.dataset["nframes"][sample_id] - 1)  #结束帧

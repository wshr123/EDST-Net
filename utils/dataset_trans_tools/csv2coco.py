...
#     """
#
#     # 构建 movie_ids，用于后续为每个 movie 分配唯一的起始编号
#     movie_ids = {}
...
#             movie_ids[name] = idx
#
#     # 存储所有视频的图像及注释信息： { movie_name: { 'img_infos': { timestamp: {...} }, 'size': (w, h) } }
#     movie_infos = {}
#
#     # 用于对 annotation 分配唯一 ID
#     global_ann_id = 1
#
#     # 遍历 CSV，每行都做一次处理
...
#
#             # 如果还未获取到该 movie 的图像尺寸，则读取一下
#             if movie_infos[movie_name]['size'] is None:
#                 # 读一张帧，获取宽高 (如有必要，可以 try/except 跳过无法读取的帧)
...
#
#             # 构造一个稳定的 image_id，防止不同 movie 的 timestamp 冲突
#             # 这里 movie_ids[movie_name] * 1000000 是为了给该 movie 预留足够大的区间
...
#                 'movie': movie_name,
#                 'timestamp': timestamp,
#                 'annotations': []  # 用列表存储 annotation，避免被 action_id 覆盖
#             }
#
#         # 从字典中取出当前图像的信息
...
#                 'timestamp': timestamp
#             })
#             all_annotations.extend(info['annotations'])
#
#     jsondata['images'] = all_images
#     jsondata['annotations'] = all_annotations
#
#     # 写入 JSON 带缩进

...
import numpy as np
from core.dataset.misc import collate_fn
import torch
import random
from PIL import Image
import torch.nn.functional as F
import core.dataset.video_transforms as T
import json
...
class VideoDataset(data.Dataset):
    ...
    def loadvideo(self, start_img, vid, frame_key):
        ...
        # print(video_frame_path + vid + '/*.jpg')
...
def make_transforms(image_set, cfg):
    ...
    print("transform image crop: {}".format(256))
    if image_set == 'train':
        return T.Compose([
            T.RandomHorizontalFlip(),
            T.RandomSizeCrop_Custom(cfg.DATA.TRAIN_CROP_SIZE),
            T.ColorJitter(),
            normalize,
        ])
    ...
    if image_set == 'val':
        return T.Compose([
            T.Resize_Custom(cfg.DATA.TEST_CROP_SIZE),
            normalize,
        ])
...
def build_dataloader(cfg):
    ...
    with open("/media/zhong/1.0T/CVPR24/tubelet-transformer/datasets/assets/assets/ava_train_v21.json", "r") as f:
        train_bbox_json = json.load(f)
    train_video_frame_bbox, train_frame_keys_list = train_bbox_json["video_frame_bbox"], train_bbox_json["frame_keys_list"]
    # print(train_frame_keys_list)   #frame list 列表形式 'qrkff49p4E4,1334',
    # print(train_video_frame_bbox)    #key:frame list  value:bboxes:, acts:  双层列表

    train_dataset = VideoDataset('/media/zhong/mypassport/ava/frames/',

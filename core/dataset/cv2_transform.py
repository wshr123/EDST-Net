...
def clip_boxes_to_image_tensor(boxes, height, width):

    boxes[:, [1, 3]] = torch.clamp(boxes[:, [1, 3]], min=0, max=width - 1)
    boxes[:, [2, 4]] = torch.clamp(boxes[:, [2, 4]], min=0, max=height - 1)
    return boxes
...
def ava_scale_boxes(size, boxes, height, width):
    if (width <= height and width == size) or (height <= width and height == size):
        return boxes
    new_width = size
    new_height = size
    if width < height:
        ...
        origin_box[:, 1:] *= scale
        boxes = origin_box
    return boxes, new_width, new_height
...
def adjust_boxes_for_crop(size, orig_height, orig_width, spatial_shift_pos, boxes):
    """
    Adjust bounding boxes based on a spatial crop.

    Args:
        size (int): size to crop (square crop).
        orig_height (int): original height of the image.
        orig_width (int): original width of the image.
        spatial_shift_pos (int): crop position:
            0 = left/top crop, 1 = center crop, 2 = right/bottom crop.
        boxes (list): list of bounding boxes for a batch of images.
            Each element is a tensor of shape (num_boxes, 4), with each box as [xmin, ymin, xmax, ymax].

    Returns:
        adjusted_boxes (list): list of adjusted bounding boxes for the cropped images.
    """
    import math

    assert spatial_shift_pos in [0, 1, 2], "spatial_shift_pos must be 0, 1, or 2"

    y_offset = int(math.ceil((orig_height - size) / 2))
    x_offset = int(math.ceil((orig_width - size) / 2))

    # Determine crop offsets based on spatial shift position
    if orig_height > orig_width:
        if spatial_shift_pos == 0:  # Top crop
            y_offset = 0
        elif spatial_shift_pos == 2:  # Bottom crop
            y_offset = orig_height - size
    else:
        if spatial_shift_pos == 0:  # Left crop
            x_offset = 0
        elif spatial_shift_pos == 2:  # Right crop
            x_offset = orig_width - size

    # Adjust bounding boxes
    if boxes is not None:
        origin_box = boxes
        origin_box[:, [1, 3]] -= x_offset  # Adjust x-coordinates
        origin_box[:, [2, 4]] -= y_offset  # Adjust y-coordinates
        boxes = origin_box
    return boxes

def detrbox2oribox(bbox, original_size, resized_size):
    import torch
    bbox_norm = bbox
    ...
    # bbox_norm[: ,[0,2]] *= W_resized
    # bbox_norm[: ,[1,3]] *= H_resized
    # bbox_norm[: ,[0,2]] /= scale_x
    # bbox_norm[: ,[1,3]] /= scale_y
    bbox_norm[: ,[1,3]] *= 1920
    bbox_norm[: ,[2,4]] *= 1080
    bbox = bbox_norm
    return bbox

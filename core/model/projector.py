...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5, act='silu'):
        ...
        self.add = shortcut and c1 == c2    #判断是否做shotcut，必须要输入c等于输出c才可以
...
class C2f(nn.Module):
    ...
    def forward(self, x):
        ...
        y = list(self.cv1(x).split((self.c, self.c), 1))#2x2,128,40,40
...
class MultiScaleProjector(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            ...
            if len(feat_fuse) > 1:
                feat_fuse = torch.cat(feat_fuse, dim=1) #midium的没有用到这个，相当于直接cat 2,1536,40,40
            else:
                feat_fuse = feat_fuse[0]
            results.append(stage(feat_fuse))    #2,256,40,40

...
from core.utils.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       accuracy, get_world_size,
                       is_dist_avail_and_initialized)

from .build import build_spatio_backbone
...
class LWDETR(nn.Module):
    ...
    def __init__(self,
        ...
        query_dim=4
        self.refpoint_embed = nn.Embedding(num_queries * group_detr, query_dim) #创建两个可以学习的emb
        ...
        self.class_embed.bias.data = torch.ones(num_classes) * bias_value   #为每一个类别生成一个bias
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        features, poss = self.backbone(samples)#[[2,40,40],[2,256,40,40]]  2,256,40,40
        ...
        hs, ref_unsigmoid, hs_enc, ref_enc = self.transformer(
            srcs, masks, poss, refpoint_embed_weight, query_feat_weight)    #srcs=feat todo
...
def build(args):
    ...
    backbone = build_spatio_backbone(args)     #todo here

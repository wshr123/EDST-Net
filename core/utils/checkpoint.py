...
import torch
from core.utils.c2_model_loading import get_name_convert_func
from core.utils.env import checkpoint_pathmgr as pathmgr
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        checkpoint["model_state"] = {"temporal_backbone." + k: v for k, v in checkpoint["model_state"].items()}  # todo
        ...
        if use_detr_pretrian:
            ...
            #                         map_location='cpu')
            checkpoint = torch.load(cfg.TRAIN.CHECKPOINT_LWDETR, map_location='cpu')
